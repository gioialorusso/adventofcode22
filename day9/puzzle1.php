...
class PositionTrackingKnot extends Knot{
    ...
    private function notInTheSameRowOrColumn(Knot $head){
        if($head->getY() != $this->getY() && $head->getX() != $this->getX()){
            return true;
        }
    }
    ...
    public function follow( Knot $head, string $direction ){
        ...
        if($distance > 1.5){
            if($this->notInTheSameRowOrColumn($head)){
                $this->moveDiagonally($head);
            }else {
                //I need to move as well in the same direction
                $this->move($direction);
            }
        }
...
$head = new Knot(0, 0);
$tail = new PositionTrackingKnot(0, 0);
...
echo $tail->countVisitedPositions();
